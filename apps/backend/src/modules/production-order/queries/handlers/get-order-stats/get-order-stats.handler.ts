...
export class GetOrderStatsHandler implements IQueryHandler<
  GetOrderStatsQuery,
  OrderStats
> {
  ...
  async execute(): Promise<OrderStats> {
    const countOf = (status?: ProductionOrderStatus) =>
      this.queryBus
        .execute<DirectusListItemsQuery, DirectusListResponse<{ id: string }>>(
          new DirectusListItemsQuery(this.collection, {
            filter: status
              ? { status: { [DirectusOperator.EQ]: status } }
              : undefined,
            fields: ['id'],
            limit: 1,
