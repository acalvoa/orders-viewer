...
interface Props {
  ...
  proposalMap: Map<string, RescheduleProposal>;
}

export default memo(function ProposalCard({ proposal, conflictsWith, proposalMap }: Props) {
  return (
      ...
      <Flex justify="space-between" align="flex-start" className="mb-6">
        <div>
          <Flex align="center" gap={8} className="mb-1.5">
            <Text strong className="text-sm text-[#001450]">{proposal.reference}</Text>
            <StatusTag status={proposal.status} />
          </Flex>
          <Text type="secondary" className="text-xs">{proposal.product}</Text>
        </div>
        <Flex align="center" gap={12}>
          <Text type="secondary" className="text-xs">
            Cant: <Text strong className="text-[#222]">{proposal.quantity}</Text>
          </Text>
          {conflictsWith.length > 0 && (
            <Flex align="center" gap={5}>
              <LinkOutlined className="text-[#FF4D4F] text-xs" />
              {conflictsWith.map((id) => {
                const partner = proposalMap.get(id);
                return (
